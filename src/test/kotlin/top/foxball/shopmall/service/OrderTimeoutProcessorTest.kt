...
class OrderTimeoutProcessorTest {
    ...
    fun `winner of timeout transition restocks and schedules payment compensation`() {
        ...
        val item = OrderItem(productId = 30, variantId = 3, sku = "TIMEOUT-3", quantity = 2)
        `when`(orderRepository.findById(10)).thenReturn(Optional.of(order))
        `when`(itemRepository.findAllByOrder_IdOrderByVariantIdAsc(10)).thenReturn(listOf(item))
        `when`(
        ...
        `when`(productVariantRepository.restock(3, 2)).thenReturn(1)

        processor.cancelExpired(10)

        verify(productVariantRepository).restock(3, 2)
    ...
    fun `loser of timeout transition does not restock twice`() {
        ...
        `when`(itemRepository.findAllByOrder_IdOrderByVariantIdAsc(11)).thenReturn(
            listOf(OrderItem(productId = 40, variantId = 4, sku = "TIMEOUT-4", quantity = 1)),
        )
        `when`(
            orderRepository.markCancelled(
                11,
        ...
        verify(productVariantRepository, never()).restock(4, 1)
