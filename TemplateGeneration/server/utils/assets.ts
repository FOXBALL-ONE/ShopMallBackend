...
export function listAssets(projectId: string, type?: string, query?: string) {
  ...
  if (query?.trim()) {
    clauses.push('(name LIKE ? OR code LIKE ? OR description LIKE ? OR tags_json LIKE ?)')
    const normalized = `%${query.trim()}%`
    params.push(normalized, normalized, normalized, normalized)
  }
  const rows = getDatabase().prepare(`
    SELECT asset_library.id, asset_library.project_id, asset_library.file_id, asset_library.type, asset_library.name, asset_library.code, asset_library.description, asset_library.tags_json, asset_library.authorization_status, asset_library.created_at, asset_library.updated_at,
    ...
    WHERE ${clauses.map((clause) => clause.replace('project_id', 'asset_library.project_id').replace('type =', 'asset_library.type =').replace('name LIKE', 'asset_library.name LIKE').replace('code LIKE', 'asset_library.code LIKE').replace('description LIKE', 'asset_library.description LIKE').replace('tags_json LIKE', 'asset_library.tags_json LIKE')).join(' AND ')}
...
export function getAsset(assetId: number, projectId?: string) {
  ...
  const row = getDatabase().prepare(`SELECT id, project_id, file_id, type, name, code, description, tags_json, authorization_status, created_at, updated_at FROM asset_library WHERE ${clauses.join(' AND ')}`).get(...params) as AssetRow | undefined
  if (!row) throw createError({statusCode: 404, statusMessage: '素材不存在。'})
  return row
}

export function createAsset(input: {projectId: string; fileId: number; type: AssetType; name: string; code: string; description: string; tags: string[]; authorizationStatus?: string | null}) {
  ...
  try {
    const result = database.prepare("INSERT INTO asset_library (project_id, file_id, type, name, code, description, tags_json, authorization_status, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, strftime('%Y-%m-%dT%H:%M:%f', 'now', 'localtime'))").run(input.projectId, input.fileId, input.type, input.name.trim(), input.code.trim(), input.description.trim(), JSON.stringify(input.tags.slice(0, 30)), input.authorizationStatus ?? null)
...
export async function deleteAsset(assetId: number, projectId: string) {
  ...
  try {
    ...
    database.prepare('INSERT OR IGNORE INTO asset_library (id, project_id, file_id, type, name, code, description, tags_json, authorization_status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(row.id, row.project_id, row.file_id, row.type, row.name, row.code, row.description, row.tags_json, row.authorization_status, row.created_at, row.updated_at)
