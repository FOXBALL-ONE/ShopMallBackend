...
type Asset = { id: number; type: Exclude<AssetType, 'ALL'>; name: string; code: string; description: string; palette: string; status?: '已确认授权' | '授权待确认' }
...
const assets = ref<Asset[]>([
  { id: 1, type: 'GARMENT', name: '月光三角杯', code: 'NW-2601', description: '象牙白 · 法式蕾丝', palette: 'cream' },
  { id: 2, type: 'GARMENT', name: '午夜束腰套装', code: 'NW-2602', description: '黑色 · 轻薄网纱', palette: 'charcoal' },
  { id: 3, type: 'MODEL', name: 'Mia Zhou', code: 'MODEL-014', description: '正面半身 · 中性妆容', palette: 'taupe', status: '已确认授权' },
  { id: 4, type: 'MODEL', name: 'Evelyn Xu', code: 'MODEL-021', description: '三分之二侧身 · 自然光', palette: 'stone', status: '授权待确认' },
  { id: 5, type: 'REFERENCE', name: '柔光棚拍参考', code: 'REF-008', description: '低对比 · 米灰背景', palette: 'sand' },
  { id: 6, type: 'REFERENCE', name: '黑色丝绸质感', code: 'REF-012', description: '高光细节 · 近景', palette: 'black' },
])
...
function saveAsset() {
  const id = Math.max(...assets.value.map((asset) => asset.id)) + 1
  const type: Asset['type'] = uploadType.value === '服装' ? 'GARMENT' : uploadType.value === '模特' ? 'MODEL' : 'REFERENCE'
  assets.value.unshift({ id, type, name: '新上传素材', code: `NEW-${String(id).padStart(3, '0')}`, description: '等待补充信息', palette: type === 'GARMENT' ? 'cream' : 'stone', status: type === 'MODEL' ? '授权待确认' : undefined })
  ...
  <div class="workspace-layout">
    ...
    <section class="main-area">
      ...
      <main class="content">
        ...
        <section class="toolbar"><div class="filter-tabs"><button v-for="filter in filters" :key="filter.value" type="button" :class="{ active: typeFilter === filter.value }" @click="typeFilter = filter.value">{{ filter.label }}</button></div><label class="search-field"><span>⌕</span><input v-model="query" type="search" placeholder="搜索名称、款号或标签"></label></section>
        <section v-if="visibleAssets.length" class="asset-grid"><article v-for="asset in visibleAssets" :key="asset.id" class="asset-card"><div class="asset-preview" :class="`palette-${asset.palette}`"><span class="asset-type">{{ assetLabel(asset.type) }}</span><div class="asset-shape" :class="asset.type.toLowerCase()"><i /><i /><i /></div><b>{{ asset.code }}</b></div><div class="asset-details"><small>{{ assetLabel(asset.type) }}</small><h2>{{ asset.name }}</h2><p>{{ asset.description }}</p><div><span v-if="asset.status" class="authorization" :class="{ pending: asset.status === '授权待确认' }">{{ asset.status === '已确认授权' ? '✓ ' : '! ' }}{{ asset.status }}</span><button type="button" @click="removeAsset(asset.id)">删除</button></div></div></article></section>
    ...
    <div v-if="uploadOpen" class="modal-backdrop" @click.self="uploadOpen = false"><form class="modal-card" @submit.prevent="saveAsset"><div class="modal-head"><div><p class="eyebrow">NEW ASSETS</p><h2>上传项目素材</h2></div><button type="button" class="close-button" aria-label="关闭" @click="uploadOpen = false">×</button></div><label>素材类型<select v-model="uploadType"><option>服装</option><option>模特</option><option>姿势参考</option><option>场景参考</option></select></label><label class="drop-zone"><span class="upload-symbol">↑</span><strong>选择或拖入图片</strong><input type="file" accept="image/jpeg,image/png,image/webp"><small>JPG / PNG / WebP · 单张不超过 25MB · 最多 20 张</small></label><div class="form-grid"><label>内部名称<input placeholder="例如：月光三角杯"></label><label>款号<input placeholder="NW-2601"></label><label>品类<input placeholder="文胸 / 套装"></label><label>颜色<input placeholder="象牙白"></label><label>材质<input placeholder="法式蕾丝"></label><label>标签<input placeholder="新品, 主推款"></label></div><label v-if="uploadType === '模特'" class="check-line"><input type="checkbox"> 我确认模特素材已获得品牌生产使用授权</label><button class="dark-button modal-submit" type="submit">保存到素材库</button></form></div>
