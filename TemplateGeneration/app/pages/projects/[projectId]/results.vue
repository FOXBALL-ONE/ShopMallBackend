  ...
  <div class="results-layout">
    ...
    <section class="results-main">
      ...
      <main class="results-content">
        ...
        <section v-if="shownResults.length" class="result-grid"><article v-for="result in shownResults" :key="result.id" class="result-card" :class="{ selected: selectedIds.includes(result.id) }"><button class="result-media" type="button" @click="toggleSelected(result.id)"><div v-if="result.generationStatus === 'READY' && result.file" class="result-art"><img :src="result.file.downloadUrl" :alt="result.workflow"></div><div v-else class="result-art" :class="`palette-${paletteFor(result)}`"><span>{{ result.generationStatus === 'FAILED' ? 'FAILED' : result.generationStatus === 'CANCELLED' ? 'CANCELLED' : `${result.stage} · ${result.progress}%` }}</span><i /></div><em class="review-badge" :class="result.status.toLowerCase()">{{ result.generationStatus === 'READY' ? statusLabel(result.status) : result.generationStatus === 'FAILED' ? '生成失败' : result.generationStatus === 'CANCELLED' ? '已取消' : '生成中' }}</em></button><div class="result-info"><small>{{ result.media }} · {{ result.workflowVersion || `V${result.version}` }}</small><h3>{{ result.workflow }}</h3><div class="result-times"><span><b>提交时间</b><time :datetime="result.createdAt" :title="result.createdAt">{{ formatDateTime(result.createdAt) }}</time></span><span><b>生成时间</b><time :datetime="result.generatedAt || undefined" :title="result.generatedAt || '尚未生成完成'">{{ formatDateTime(result.generatedAt) }}</time></span></div><p>{{ result.generationStatus === 'FAILED' ? (result.errorMessage || '生成失败') : result.prompt || '该结果暂无提示词记录。' }}</p><div><button type="button" @click="activeResult = result">查看详情</button><button type="button" :disabled="result.generationStatus !== 'READY'" @click="toggleSelected(result.id)">{{ selectedIds.includes(result.id) ? '取消选择' : '选择对比' }}</button></div></div></article></section>
...
.result-art img { display: block; width: 100%; height: 100%; object-fit: contain; }.modal-art img { display: block; width: auto; max-width: 100%; height: auto; max-height: min(68vh, 720px); object-fit: contain; }.result-times { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin: 9px 0 7px; }.result-times span { display: flex; flex-direction: column; gap: 2px; min-width: 0; }.result-times b { color: #9b958d; font-size: 7px; font-weight: 400; }.result-times time { color: #6f6961; font-size: 7px; white-space: nowrap; }
