...
type ResultItem = {
  ...
  prompt: string
  uri: string | null
  file: {downloadUrl: string; contentType: string} | null
...
async function loadResults() {
  ...
  try {
    ...
    if (activeProjectId.value === requestProjectId) results.value = response.results
  } catch (error: unknown) {
...
<template>
  <div class="results-layout">
    ...
    <div v-if="activeResult" class="modal-backdrop" @click.self="activeResult = null"><section class="result-modal"><div class="modal-head"><div><p class="eyebrow">RESULT DETAIL</p><h2>{{ activeResult.workflow }}</h2></div><button class="close-button" type="button" aria-label="关闭" @click="activeResult = null">×</button></div><div v-if="activeResult.generationStatus === 'READY' && activeResult.file" class="modal-art"><img :src="activeResult.file.downloadUrl" :alt="activeResult.workflow"></div><div v-else class="modal-art" :class="`palette-${paletteFor(activeResult)}`"><i /><span>{{ activeResult.stage }} · {{ activeResult.progress }}%</span></div><dl><div><dt>生成状态</dt><dd>{{ activeResult.generationStatus === 'READY' ? '已完成' : activeResult.generationStatus === 'FAILED' ? '生成失败' : activeResult.generationStatus === 'CANCELLED' ? '已取消' : `${activeResult.stage} · ${activeResult.progress}%` }}</dd></div><div><dt>审核状态</dt><dd>{{ statusLabel(activeResult.status) }}</dd></div><div><dt>工作流版本</dt><dd>{{ activeResult.workflow }} · {{ activeResult.workflowVersion || `V${activeResult.version}` }}</dd></div><div><dt>生成类型</dt><dd>{{ activeResult.media === 'VIDEO' ? '视频' : '图片' }}</dd></div><div><dt>提示词</dt><dd>{{ activeResult.prompt || '暂无提示词记录。' }}</dd></div></dl><div class="modal-actions"><button class="quiet-button" type="button" :disabled="reviewSaving || activeResult.generationStatus !== 'READY'" @click="updateReview(activeResult!, 'REJECTED')">驳回</button><button class="dark-button" type="button" :disabled="reviewSaving || activeResult.generationStatus !== 'READY'" @click="updateReview(activeResult!, 'APPROVED')">通过并记录</button></div></section></div>
...
.result-art img, .modal-art img { display: block; width: 100%; height: 100%; object-fit: cover; }
