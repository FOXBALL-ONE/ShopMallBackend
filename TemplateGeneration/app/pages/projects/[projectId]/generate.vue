...
async function deleteTask(task: QueueTask) {
  if (!['已完成', '已取消', '生成失败'].includes(task.status) || deletingTaskIds.value.includes(task.id)) return
  if (!window.confirm(`确定删除任务 ${task.id}“${task.name}”吗？任务记录及其生成结果将被永久删除。`)) return
  const requestProjectId = activeProjectId.value
  deletingTaskIds.value.push(task.id)
  try {
    await $fetch(`/api/projects/${encodeURIComponent(requestProjectId)}/generation-tasks/${task.id}`, {method: 'DELETE'})
    if (activeProjectId.value !== requestProjectId) return
    queue.value = queue.value.filter((item) => item.id !== task.id)
    showToast(`任务 ${task.id} 已删除`)
  } catch (error: unknown) {
    if (activeProjectId.value === requestProjectId) showToast(requestError(error, '任务删除失败，请重试'))
  } finally {
          ...
          <section class="queue-panel">
            ...
            <div class="queue-table" role="table" aria-label="任务队列">
              ...
              <div v-for="task in queue" :key="task.id" class="queue-record" role="row">
                ...
                <span class="task-actions"><button v-if="!['已完成', '已取消', '生成失败'].includes(task.status)" type="button" @click="cancelTask(task)">取消</button><button v-else type="button" :disabled="deletingTaskIds.includes(task.id)" @click="deleteTask(task)">{{ deletingTaskIds.includes(task.id) ? '删除中' : '删除' }}</button></span>
              </div>
